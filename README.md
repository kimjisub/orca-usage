...
- **Scores every account and shows the working.** Four metrics are normalised and weighted into one number: what will be wasted at the reset, how hard the account must be burned to finish before it, how much can be spent in the next five hours, and how much of the week is left. The 판정 tab draws each account's score as a stacked bar with the per-metric contributions, and the weights are editable in settings, so the ranking and the switch decision can be tuned and inspected rather than guessed at.
